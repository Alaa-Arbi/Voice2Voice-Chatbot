...
class VoiceChatbot:
    ...
    async def capture_audio(self):
        ...
        try:
            while self.is_active:
                raw_audio_data = stream.read(CHUNK_SIZE, exception_on_overflow=False) # -> bytes
                await self.send_audio_chunk(raw_audio_data)
...
async def main():
    ...
    playback_task = asyncio.create_task(chatbot.play_audio())
    
    # Start audio capture task
    capture_task = asyncio.create_task(chatbot.capture_audio())
    ...
    if chatbot.response and not chatbot.response.done():
        chatbot.response.cancel()
